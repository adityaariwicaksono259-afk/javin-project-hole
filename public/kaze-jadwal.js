...
function isJadwalTV(d){
  ...
  var f = d.data[0];
  return !!(f && f.channel && Array.isArray(f.jadwal));
}
...
function renderJadwal(d){
  ...
  arr.forEach(function(ch){
    ...
    h += '<div style="font-size:13px;font-weight:700;color:#22d3ee;letter-spacing:.5px">' + esc(channel) + '</div>';
    h += '<div style="margin-left:auto;font-size:10px;color:#64748b;font-family:ui-monospace,monospace">' + jadwal.length + ' acara</div>';
    h += '</div>';
