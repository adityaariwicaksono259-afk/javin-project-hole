  ...
  fetch(url)
    ...
    .then(function(data){
      ...
      Object.keys(data).forEach(function(k){
        var v = data[k];
        if (typeof v === 'string' && !seen[v]) { seen[v] = true; items.push({ key: k, file: v }); }
      });
      var h = '';
      ...
      if (tab === 'effects') h += '💡 ' + items.length + ' efek Alight Motion. Klik untuk download XML efek.';
      else h += '💡 ' + items.length + ' shape Alight Motion. Klik untuk download XML shape.';
      h += '</div>';
      h += '<div style="display:grid;grid-template-columns:repeat(2,1fr);gap:8px">';
      items.slice(0, 200).forEach(function(item){
        var name = item.key.replace(/^com\.alightcreative\.(effects\.|blend\.|shape\.)?/i, '').replace(/[-_]/g, ' ');
        var dlUrl = AM_BASE + '/runtime/' + encodeURIComponent(item.file);
...
function renderAMPresets(d){
  ...
  h += '<button class="kz-am-tab active" data-tab="presets" style="flex:1;padding:12px 8px;border:0;background:transparent;color:#22d3ee;font-weight:700;font-size:12px;cursor:pointer;font-family:inherit;border-bottom:2px solid #22d3ee;white-space:nowrap">📦 Preset (' + presets.length + ')</button>';
  ...
  if (images.length) {
    h += '<div style="font-size:11px;color:#22d3ee;font-weight:700;text-transform:uppercase;letter-spacing:.5px;margin-bottom:8px">🖼️ Thumbnails</div>';
    ...
    images.slice(0, 12).forEach(function(img){
      var imgUrl = AM_BASE + '/preset-images/' + encodeURIComponent(img.name);
  ...
  presets.forEach(function(p){
    ...
    var dlUrl = AM_BASE + '/preset/' + encodeURIComponent(fname);
    ...
    h += '</div>';
    h += '<a href="' + esc(dlUrl) + '" download target="_blank" rel="noopener" style="padding:8px 14px;background:linear-gradient(135deg,#0EA5E9,#22d3ee);border-radius:8px;color:#06111f;font-size:11px;font-weight:700;text-decoration:none;flex-shrink:0">⬇️</a>';
    h += '</div>';
  });
  h += '</div>';
  h += '</div>';
...
console.log('BETOx1: KazeAM siap');
})();
